...
def test_cluster(net, device,path,epoch):
    ...
    for file in os.listdir(path):
        ...
        for key in true_cluster:
            gold[int(list(true_cluster[key])[0])].append(key)
    ...
    with open("test_output_simple/test_1"+str(epoch)+"_cluster.json", 'w') as file:
...
def main():
    ...
    net = Net().to(device)
    #net.load_state_dict(torch.load('pair_net.ckpt'))
    ...
    train, label = read_data('merge_group_1/train/')
    val, val_label = read_data('merge_group_1/val/')
    test, test_label = read_data('merge_group_1/test/')
    ...
    for epoch in range(30):  # loop over the dataset multiple times
        ...
        test_cluster(net,device,'merge_group_1/test/',epoch)
        path = 'ckpt/merge_'+str(epoch)+'.ckpt'
        torch.save(net.state_dict(), path)
    #test_cluster(net,device,'pair_sentence_1/test/',0)
